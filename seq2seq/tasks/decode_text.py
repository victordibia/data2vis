...
class DecodeText(InferenceTask):
  ...
  """
    ...
    def before_run(self, _run_context):
        ...
        fetches["features.source_len"] = self._predictions[
            "features.source_len"]
    ...
    def run_through_beam(self, predicted_tokens):
        ...
        if (np.ndim(predicted_tokens) == 1):
            out_val = self.params["delimiter"].join(predicted_tokens).split(
                "SEQUENCE_END")[0]
            beam_output_array.append(out_val)
        else:
            for i in range(predicted_tokens.shape[-1]):
                out_val = self.params["delimiter"].join(
                    predicted_tokens[:, i]).split("SEQUENCE_END")[0]
                # print (">>>>> ", out_val)
                beam_output_array.append(out_val)
