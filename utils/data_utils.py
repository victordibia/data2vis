...
def generate_field_types(t_data): 
    ...
    for field_name in t_data[0]:
        current_label = non_null_label(t_data,field_name) #t_data[0][field_name]
...
def replace_fieldnames(source_data, field_name_types, replace_direction):
    for field_name in field_name_types:
        if(replace_direction):
            source_data = str(source_data).replace(str(field_name), field_name_types[field_name])
        else:
            source_data = str(source_data).replace(str(field_name_types[field_name]), field_name)
    return source_data
...
"""
def generate_data_pairs(examples_directory,train_data_output_directory, data_split_params):
    all_sources_hold = []
    all_target_hold = []
            ...
            if filepath.endswith("vl.json"): 
                ...
                if ("url" in data["data"]):
                    ...
                    for i in range(0, num_samples_per_example): 
                        ...
                        for field_name in field_name_types:
                            target_vega_spec = target_vega_spec.replace(str(field_name), field_name_types[field_name])
                            source_data_spec = source_data_spec.replace(str(field_name), field_name_types[field_name])
    ...
    for param in data_split_params:
        with open(train_data_output_directory + "/" + param["tag"] + ".sources", mode='wt', encoding='utf-8') as outfile:
                   outfile.write('\n'.join(str(line) for line in all_sources_hold[ int(param["percentage"][0]*len(all_sources_hold)) : int(param["percentage"][1]* len(all_sources_hold)) ]))
        print("  > Saved ",train_data_output_directory + "/" + param["tag"] + ".sources" )
        with open(train_data_output_directory + "/" + param["tag"] + ".targets", mode='wt', encoding='utf-8') as outfile:
                   outfile.write('\n'.join(str(line) for line in all_target_hold[ int(param["percentage"][0]*len(all_target_hold)) : int(param["percentage"][1]* len(all_target_hold)) ]))
        print("  > Saved ",train_data_output_directory + "/" + param["tag"] + ".targets" )
...
def clean_examples_directory(examples_directory):
    print("Cleaning example files. Removing png, csv and other file types from example directory.")
    vl_data_list = []
    for subdir, dirs, files in os.walk(examples_directory):
        for file in files: 
            filepath = subdir + os.sep + file 
            ...
            if filepath.endswith("json"):
                ...
                for item in data_content: 
                    ...
                    if (i > max_data_slice_size):
                        break
                    i = i + 1 
                print("=======", filepath, len(data_content), max_data_slice_size)
...
def load_test_dataset():
    if not (os.path.exists(test_data_list)):
        print("Test data list does not exists. Creating it now at", test_data_list)
        file_list = []
...
def clean_test_dataset():
    for subdir, dirs, files in os.walk(test_dataset_directory):
            for file in files: 
                filepath = subdir + os.sep + file
                if filepath.endswith("json") and not filepath.endswith("lsit.json"): 
                    data = json.load(open(filepath))
                    ...
                    if (len(data) == 0):
                        ...
                        if (len(first_row) < 4):
                            ...
                            if (len(data) > max_test_data_length):
                                print("File has more than ", max_test_data_length, " elements. Reducing ..")
                            ...
                            if (data_file_size > max_testdata_file_size):
                                print("Data file greater than max. Deleting Example", data_file_size)
                                os.remove(filepath)
...
def profile_dataset_vegaspec(examples_directory):
    mark_types = []
    transformation_types = ["bin","aggregate", "calculate", "filter", "timeUnit","sort"]
    transform_counts = []
    for subdir, dirs, files in os.walk(examples_directory):
            for file in files: 
                filepath =   os.path.join(subdir, file)
...
def forward_norm(source_data, destination_file, f_names ):
    ...
    source_data_first_sample = replace_fieldnames(source_data_first_sample, f_names, True)
