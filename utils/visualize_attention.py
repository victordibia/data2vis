...
def _create_figure(predictions_dict):
  ...
  # Plot
  plot_data = predictions_dict["attention_scores"][:prediction_len, :source_len]
  ...
  print("".join([str(row) for row in source_words]).replace("SEQUENCE_END",""))
  print("========")
  print("".join([str(row) for row in target_words]).replace("SEQUENCE_END",""))
